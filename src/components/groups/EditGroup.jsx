...
function EditGroup({ group, onClose, onUpdate }) {
  ...
  const [groupImagePreview, setGroupImagePreview] = useState(group.group_image ? `${process.env.REACT_APP_API_URL}/${group.group_image}` : null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post(`/api/dashboard/groups/${group.id}/update`, formDataToSubmit, {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 overflow-auto">
      <div ref={modalRef} className="bg-gray-800 p-6 rounded-lg shadow-lg text-white w-3/4 max-w-lg max-h-full overflow-y-auto">
        <h2 className="text-2xl mb-4">Modifier le groupe</h2>
